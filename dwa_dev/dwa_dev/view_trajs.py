...
# Read trajectory properties to be able to iterate over them
...
def calc_dynamic_window() -> list:
    """
    Calculates the dynamic window for the robot's motion planning.

    Returns:
        list: A list containing the dynamic window values [min_acc, max_acc, -max_steer, max_steer].
    """
    Vs = [min_acc, max_acc, -max_steer, max_steer]
    dw = [Vs[0], Vs[1], Vs[2], Vs[3]]
    return dw

def rotateMatrix(a) -> np.ndarray:
    """
...
def main():
    ...
    i = [0, -1] # Only plot the fastest and slowest (maximum negative) velocities

    # Iterate through the two velocities
    for v in tqdm(range(2)):
        ...
        for a in np.arange(dw[0], dw[1]+a_resolution, a_resolution):
            ...
            for delta in delta_list:
                # Display the trajectory rotated by -45 degrees
