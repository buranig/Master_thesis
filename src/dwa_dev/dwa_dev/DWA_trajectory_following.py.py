...
def main():
    ...
    a = 2.0
    delta = -0.785398
    # delta = 0.7853983267948961
    x = [0.0, 0.0, 0.0, v, 0.0] #[x, y, yaw, v, omega]   
    ref = data[str(v)][str(a)][str(delta)]
    ref = np.array(ref)
    trajectory = np.zeros((len(ref),4))
    trajectory[:,0] = ref[:,0]
    trajectory[:,1] = ref[:,1]
    trajectory[:,2] = ref[:,2]
    trajectory[:,3] = ref[:,3]
    trajectory[:,0:2] = (trajectory[:,0:2]) @ utils.rotateMatrix(np.radians(90)-x[2]) + [x[0],x[1]]
    
    # print(time.time()-old_time)
    trajectory[:,2] = trajectory[:,2]-np.pi/2+x[2]
    trajectory[:,2] = utils.normalize_angle_array(trajectory[:,2])
    ...
    dilated_traj = LineString(zip(trajectory1[:, 0], trajectory1[:, 1])).buffer(dilation_factor)
    i = 1
    while (len(trajectory1)>2):
        # Get current state and control input
        u = [a, delta]

        x = utils.nonlinear_model_numpy_stable(x, u, dt)
        # trajectory1 = trajectory1[i:,:]
        # trajectory1[:,0:2] += [x[0]-trajectory1[0,0],x[1]-trajectory1[0,1]]
        # Rotate the whole trajectory arount the first trajectory point
        trajectory1 = trajectory1[1:,:] 
        trajectory1[:,0:2] -= trajectory1[1,0:2]
        # print(f'yaw angle: {np.rad2deg(utils.normalize_angle(x[2]))}, traejctory angle: {np.rad2deg(trajectory1[0,2])}, difference: {np.rad2deg(utils.normalize_angle(x[2]-trajectory1[0,2]))}\n')
        # print(np.rad2deg(utils.normalize_angle(x[2]-trajectory[0,2])))
        trajectory1[:,0:2] = (trajectory1[:,0:2]) @ utils.rotateMatrix(utils.normalize_angle(-x[2]+trajectory1[0,2]))
        trajectory1[:,0:2] += x[0:2]
        trajectory1[:,2] += utils.normalize_angle(x[2]-trajectory1[0,2])
        # print(f'After: yaw angle: {np.rad2deg(utils.normalize_angle(x[2]))}, traejctory angle: {np.rad2deg(trajectory1[0,2])}, difference: {np.rad2deg(utils.normalize_angle(x[2]-trajectory1[0,2]))}\n')

        i += 1

        # trajectory1[:,0:2] = (trajectory1[:,0:2]) @ utils.rotateMatrix(x[2]-trajectory1[0,2])
        ...
        plot_polygon(dilated_traj, ax=ax, add_points=False, alpha=0.5)
        plt.plot(x[0], x[1], "xr")
