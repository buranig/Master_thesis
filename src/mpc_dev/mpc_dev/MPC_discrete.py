...
class MPC_DISCRETE_algorithm():
    ...
    def calc_control_and_trajectory(self, x, v_search, ob, i):
        ...
        if len(u_buf['throttle']) > emergency_brake_distance and len(u_buf['steering']) > emergency_brake_distance:
...
def random_harem():
    ...
    with open(dir_path + '/MPC_trajectories_harem.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([trajectory, targets], f) 
    print("Saving the trajectories to /mpc_dev/mpc_dev/MPC_dilated_traj_harem.pkl")
    with open(dir_path + '/MPC_dilated_traj_harem.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
